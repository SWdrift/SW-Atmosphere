...
- 每个用例携带明确验证目标和完整可调控制规格。当前覆盖 FOV、天体场景、曝光、质量、散射阶数、调试视图与 Rayleigh/Mie/ozone 开关；新增参数先进入权威控制状态，再扩展基线和用例。
...
- 当地太阳高度由观察点 local up 与明确的当地水平参考方向构建，再转换为确定性地球轨道初相。用例表不直接编码世界方位角，也不绕过天体快照。
- 月球构图规格只声明相机射线偏移；场景生成器把该射线与月球轨道球求交并反解为轨道初相，因此 URL、快照、时间推进和普通天体面板始终消费同一种 `CelestialScenario`。
...
- 带月球的大气参考图只比较当前系统已表达的月相、轮廓和大气透射层次；云、未知镜头与裁切不得反推为实体显示尺寸或物理参数。
